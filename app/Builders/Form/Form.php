...
class Form
{
	...
	private function validate()
	{
		...
		{
			// Test the validation
			$passed = $this->validate->rules($rule, $ruleValue, $this->receivedData[$this->groupName]);

			// If passed is an array we have a fail
			if(!empty($passed))
			{
				$this->groupValidation['type'] = $passed['type'];
				$this->groupValidation['message'] = $passed['message'];
				$this->valid = $passed['valid'];
			}
		}
	}
